...
class RNNLM_Model(LanguageModel):
  ...
  def add_projection(self, rnn_outputs):
    ...
    for h in rnn_outputs:
        outputs.append(tf.matmul(h, U) + b2)
    ### END YOUR CODE
    return outputs
  ...
  def add_loss_op(self, output):
    ...
    loss = sequence_loss([output], [tf.reshape(self.labels_placeholder, [-1])], ones, len(self.vocab))
    ### END YOUR CODE
    return loss
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('RNN') as scope:
      ...
      for i, input in enumerate(inputs):
        ...
        d_input = tf.nn.dropout(input, self.dropout_placeholder)
        h = tf.nn.sigmoid(tf.matmul(h, H) + tf.matmul(d_input, I) + b1)
        d_output = tf.nn.dropout(h, self.dropout_placeholder)
        rnn_outputs.append(d_output)
