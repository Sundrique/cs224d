...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    sigmoids = sigmoid(U.dot(predicted) * labels)
    cost = - np.sum(np.log(sigmoids))

    delta = sigmoids - 1
    gradPred = (delta * labels).reshape(1, sigmoids.shape[0]).dot(U).flatten()

    grad = np.zeros(outputVectors.shape)

    grad[target] = (delta[0] * predicted).flatten()

    for i in range(1, K + 1):
        grad[indices[i]] -= (delta[i] * predicted).flatten()
